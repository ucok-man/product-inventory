...
import { createProductSchema, type updateProductSchema } from "@/lib/schemas";
...
export default function UpdateProductForm(props: Props) {
  const [amountDisplay, setAmountDisplay] = React.useState(
    formatNumber(String(props.product.amount)),
  );
  const [qtyDisplay, setQtyDisplay] = React.useState(
    formatNumber(String(props.product.qty)),
  );
  ...
  const { mutate, isPending } = api.product.update.useMutation({
    ...
    onSuccess: () => {
      toast.success("Success", {
        description: "The product has been updated from inventory.",
      });
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const payload: z.infer<typeof updateProductSchema> = {
        ...value,
        id: props.product.id,
      };
      if (value.name.trim() === props.product.name) {
        payload.name = undefined;
      }
      if (value.description.trim() === props.product.description) {
        payload.description = undefined;
      }
      if (value.amount === props.product.amount) {
        payload.amount = undefined;
      }
      if (value.qty === props.product.qty) {
